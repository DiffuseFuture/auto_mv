...
## 2. 预置条件与配置
- **API Key 获取**: 引导用户前往 [http://127.0.0.1:5173](http://127.0.0.1:5173) 获取。
...
### 4.1 上传音频接口 (Upload)
- **URL**: `POST http://127.0.0.1:5173/ohyesai-next/api/vio/skill/upload?apikey={{OHEYSAI_API_KEY}}`
- **Payload**: `multipart/form-data` (字段名: `file`)
- **功能**: 将本地音频转换为 `audioFileId`。

### 4.2 提交任务接口 (Submit)
- **URL**: `POST http://127.0.0.1:5173/ohyesai-next/api/vio/skill/mv-submit?apikey={{OHEYSAI_API_KEY}}`
- **Request Body**:
  ```json
  {
    "audioFileId": "string",
...
### 4.3 查询任务接口 (Query)
- **URL**: `GET http://127.0.0.1:5173/ohyesai-next/api/vio/skill/mv-query?apikey={{OHEYSAI_API_KEY}}&taskId={{taskId}}`
- **Response Structure**:
  ```json
  {
    "code": 0,
