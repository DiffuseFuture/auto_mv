<template>
  <el-dialog
      ...
      @close="handleCancel"
  >
    <div class="music-trimmer-wrapper w-full">
      <div class="bg-[#1A1A1A] rounded-[16px] p-4 border border-white/10 shadow-2xl overflow-visible">
        <!-- 波形轨道 -->
        <div
            ref="waveContainerRef"
            class="flex items-center h-[90px] bg-[#2B2B2B] rounded-[14px] overflow-visible mb-3 select-none"
        >
          <!-- 播放按钮 -->
          <button
              class="w-10 h-10 flex-center text-white hover:text-[#C2FF00] transition-colors cursor-pointer outline-none focus:outline-none focus-visible:outline-none focus-visible:ring-0"
              @click="togglePreview"
              :disabled="!previewUrl"
              :class="!previewUrl ? 'opacity-40 cursor-not-allowed' : ''"
          >
            <svg-icon :name="isPlaying ? 'gy-pause' : 'gy-play2'" size="26" color="currentColor"/>
          </button>

          <div
              ref="trackRef"
              class="relative h-full flex-1 mx-4"
          >
            <!-- 真实波形 -->
            ...
            <div class="absolute inset-0 z-10">
              ...
              <div
                  ...
                  :class="(isRangeDragging || isSeekingPlayhead) ? 'cursor-grabbing shadow-[0_0_0_1px_rgba(194,255,0,0.6),0_0_10px_rgba(194,255,0,0.25)]' : 'cursor-grab'"
                  :style="trimRangeStyle"
                  @mousedown.stop.prevent="startDragRange"
              ></div>

              <!-- 播放指针 -->
              <div
                  v-if="showPlayhead"
                  class="absolute top-0 bottom-0 z-20 w-[2px] bg-[#C2FF00] shadow-[0_0_8px_rgba(194,255,0,0.8)] cursor-ew-resize"
                  :style="{ left: `calc(${playheadPercent}% - 1px)` }"
                  @mousedown.stop="startSeekPlayhead"
                  @click.stop
              >
                <div
                    class="absolute -top-[8px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-r-[6px] border-t-[8px] border-l-transparent border-r-transparent border-t-[#C2FF00]"
                ></div>
                <div
                    class="absolute -top-[10px] left-[8px] z-30 px-[6px] py-[1px] rounded-full bg-[#5A5A5A]/80 border border-white/20 text-white text-[12px] leading-[16px] font-mono whitespace-nowrap"
                >
              ...
              <div
                  class="absolute top-0 bottom-0 w-[16px] bg-[#C2FF00] rounded-l-[14px] flex-center cursor-ew-resize"
                  :style="{ left: `calc(${startPercent}% - 16px)` }"
                  @mousedown.stop="startDrag('start')"
                  @click.stop
              >
                <div class="w-[2px] h-[40px] bg-black/60 rounded-full"></div>
              </div>

              <div
                  class="absolute top-0 bottom-0 w-[16px] bg-[#C2FF00] rounded-r-[14px] flex-center cursor-ew-resize"
        ...
        <div class="flex items-center justify-between px-1">
          <div class="text-[#C2FF00] text-[16px] font-medium">
            {{ t('creation.musicTrimmer.selectedDuration') }}{{ formatDuration(displaySelectedDuration) }}
          </div>
          <div class="bg-[#2B2B2B] rounded-[10px] px-3 py-1 border border-white/10">
            <span class="text-[#C2FF00] text-[16px] font-mono">
...
const startSeekPlayhead = () => {
  ...
  document.addEventListener('mousemove', handleMouseMove);
  document.addEventListener('mouseup', stopSeekPlayhead);
};
...
const startDragRange = (e) => {
  ...
  rangeDragStartX.value = e.clientX;
...
const handleMouseMove = (e) => {
  ...
  if (isRangeDragging.value) {
    ...
    const durationSec = snapToSecond(duration);
    const deltaSec = snapToSecond(((e.clientX - rangeDragStartX.value) / usableWidth) * duration);
  ...
  if (isSeekingPlayhead.value) {
    ...
    const ratio = (e.clientX - rect.left) / usableWidth;
  ...
  let percent = ((e.clientX - rect.left) / usableWidth) * 100;
...
const stopSeekPlayhead = async () => {
  ...
  document.removeEventListener('mousemove', handleMouseMove);
  document.removeEventListener('mouseup', stopSeekPlayhead);
